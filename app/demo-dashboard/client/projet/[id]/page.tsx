...
// Portail client — DÉMO. Reproduit le rendu de app/client/projet/[token]/page.tsx
// (mêmes sections : header artisan, statut, résumé, devis, discussion, timeline,
// formulaire de complément, anti-spam) mais sans aucun appel réseau : les
// données viennent de DemoModeContext (fictives, locales), et toute action
// (envoi d'infos, message, acompte) est simulée. Jamais de vraie écriture,
...
const inputStyle: React.CSSProperties = {
  width: '100%', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '12px',
  fontSize: '14px', fontFamily: 'inherit', color: '#111827', boxSizing: 'border-box',
};
...
export default function DemoClientPortalPage() {
  ...
  const { projects, artisan, clientEvents, addClientEvent } = useDemoMode();
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
        <div style={{ textAlign: 'center', color: '#374151' }}>
          ...
          <p>Lien invalide ou expiré (simulation démo).</p>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = () => {
    ...
    addClientEvent(project.id, {
      ...
      title: 'Informations complétées par le client (démo)',
      message: [details, message].filter(Boolean).join(' — ') || null,
      source: 'client',
    });
    if (message.trim()) {
      addClientEvent(project.id, { type: 'client_message', title: 'Message client', message: message.trim(), source: 'client' });
    }
    setDetails('');
    setMessage('');
    setDone(true);
    setSnapshotKey(JSON.stringify({ firstName, lastName, email, phone, address, budget, timeline, details: '', message: '' }));
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9fafb', color: '#111827' }}>
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '32px 20px 64px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '28px 24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          <h1 style={{ fontSize: '20px', fontWeight: 800, margin: '0 0 4px' }}>Suivi de votre demande</h1>
          {project.createdAt && (
            <p style={{ fontSize: '12px', color: '#9ca3af', margin: '0 0 16px' }}>Demande reçue le {formatDate(project.createdAt)}</p>
          )}
          <div style={{
          ...
          <p style={{ fontSize: '14px', color: '#374151', margin: '0 0 4px' }}>Votre demande a bien été reçue.</p>
          <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>Aucune création de compte n&apos;est nécessaire.</p>
        </div>

        <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          {project.photos.length > 0 && (
            <div style={{ display: 'flex', gap: '8px', marginTop: '14px', flexWrap: 'wrap' }}>
              {project.photos.map((p, i) => (
                <img key={i} src={p.thumbnailUrl || p.url} alt="" style={{ width: '64px', height: '64px', objectFit: 'cover', borderRadius: '8px', border: '1px solid #e5e7eb' }} />
              ))}
            </div>
          )}
        </div>
        ...
        {devis && (
          <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gap: '6px', fontSize: '13px', color: '#374151', marginBottom: '14px' }}>
              <p style={{ margin: 0 }}><strong>Référence :</strong> {devis.numero}</p>
              <p style={{ margin: 0 }}><strong>Montant :</strong> {formatAmount(devis.amount)}</p>
              {devis.date_emission && <p style={{ margin: 0 }}><strong>Envoyé le :</strong> {formatDate(devis.date_emission)}</p>}
              {devis.accepted && devis.accepted_at && <p style={{ margin: 0 }}><strong>Accepté le :</strong> {formatDate(devis.accepted_at)}</p>}
              {devis.declined && devis.declined_at && <p style={{ margin: 0 }}><strong>Refusé le :</strong> {formatDate(devis.declined_at)}</p>}
              {devis.declined && devis.decline_reason && <p style={{ margin: 0 }}><strong>Motif :</strong> {devis.decline_reason}</p>}
            </div>
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '12px' }}>
              <span style={{
                display: 'inline-block', background: primaryColor, color: '#fff', fontWeight: 700,
                fontSize: '13px', padding: '10px 16px', borderRadius: '8px', opacity: 0.9,
              }}>
                📄 PDF fictif de démonstration
              </span>
            </div>
            <button
              onClick={() => {
                setDepositToast("Simulation : ceci est une démonstration. Aucun paiement réel n'est déclenché.");
                window.setTimeout(() => setDepositToast(null), 5000);
              }}
              style={{
                display: 'inline-block', background: '#111827', color: '#fff', fontWeight: 700,
                fontSize: '13px', padding: '10px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer',
              }}
            >
              {devis.amount ? `Régler l'acompte (${formatAmount(Math.round(devis.amount * 0.3))}) — démo` : "Régler l'acompte — démo"}
            </button>
            {depositToast && (
              <p style={{ marginTop: '10px', fontSize: '12px', color: '#b45309', fontWeight: 600 }}>{depositToast}</p>
            )}
          </div>
        )}
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 16px' }}>Vos échanges directs avec l&apos;artisan à propos de cette demande (démo).</p>
                ...
                return (
                  <div key={ev.id} style={{ display: 'flex', justifyContent: isClient ? 'flex-end' : 'flex-start' }}>
                    <div style={{
                      ...
                      <div style={{ fontSize: '10px', fontWeight: 700, marginBottom: '4px', color: isClient ? 'rgba(255,255,255,0.85)' : '#6b7280', textAlign: isClient ? 'right' : 'left' }}>
                        {isClient ? 'Vous' : 'Artisan'} · {formatDateTimeFr(ev.createdAt)}
                      </div>
                      <p style={{ margin: 0, fontSize: '13px', lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{ev.message}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '12px', color: '#6b7280', margin: '0 0 16px' }}>Historique visible côté client (démo).</p>
          {systemEvents.length === 0 ? (
            <p style={{ fontSize: '13px', color: '#6b7280', margin: 0 }}>Votre demande a bien été reçue. Aucun échange pour le moment.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {systemEvents.map((ev) => (
                <div key={ev.id} style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderLeft: '3px solid #6b7280', borderRadius: '10px', padding: '10px 12px' }}>
                  ...
                  {ev.message && <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#374151', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{ev.message}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        {done && (
          <div style={{ background: '#f0fdf4', border: '1px solid #22c55e', borderRadius: '12px', padding: '16px', marginBottom: '20px', color: '#15803d', fontWeight: 600, fontSize: '13px' }}>
            Simulation : vos informations ont été ajoutées au dossier démo. Aucune donnée réelle n&apos;a été modifiée.
          </div>
        )}
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#6b7280', margin: '0 0 16px' }}>
            Complétez ces informations pour voir la démo réagir (simulation, rien n&apos;est enregistré côté serveur).
          </p>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div><label style={labelStyle}>Prénom</label><input style={inputStyle} value={firstName} onChange={(e) => setFirstName(e.target.value)} maxLength={120} /></div>
            <div><label style={labelStyle}>Nom</label><input style={inputStyle} value={lastName} onChange={(e) => setLastName(e.target.value)} maxLength={120} /></div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div><label style={labelStyle}>Téléphone</label><input style={inputStyle} value={phone} onChange={(e) => setPhone(e.target.value)} maxLength={40} /></div>
            <div><label style={labelStyle}>Email</label><input style={inputStyle} type="email" value={email} onChange={(e) => setEmail(e.target.value)} maxLength={200} /></div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div><label style={labelStyle}>Budget</label><input style={inputStyle} value={budget} onChange={(e) => setBudget(e.target.value)} maxLength={120} /></div>
            <div><label style={labelStyle}>Délai souhaité</label><input style={inputStyle} value={timeline} onChange={(e) => setTimeline(e.target.value)} maxLength={200} /></div>
          </div>
          <div style={{ marginBottom: '12px' }}>
            ...
            <textarea value={details} onChange={(e) => setDetails(e.target.value)} rows={4} maxLength={4000}
              style={{ ...inputStyle, resize: 'vertical', fontFamily: 'inherit' }} placeholder="Détails, contraintes, précisions utiles..." />
          </div>
          <div style={{ marginBottom: '20px' }}>
            ...
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3} maxLength={2000}
              style={{ ...inputStyle, resize: 'vertical', fontFamily: 'inherit' }} placeholder="Un message pour l'artisan..." />
          </div>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 700, fontSize: '15px', padding: '14px', borderRadius: '10px', border: 'none',
              width: '100%', cursor: !hasChanges ? 'not-allowed' : 'pointer',
            }}
          >
            Envoyer mes informations (démo)
          </button>
          {!hasChanges && (
        ...
        <p style={{ textAlign: 'center', fontSize: '11px', color: '#9ca3af' }}>Propulsé par Kadria — mode démo, données fictives</p>
      </div>
    </div>
  );
}
