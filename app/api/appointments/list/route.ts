...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabaseAdmin
      ...
      .select('id, project_id, start_time, end_time, location, status, client_name, google_event_id, title')
    ...
    const appointments = rows
      .filter((row) => {
        ...
        // (double vérification en plus du filtre artisan_id sur la table
        // project_appointments elle-même).
        if (!row.project_id) return true
        const project = projectById.get(row.project_id)
        return !project || project.artisanId === artisanId
      })
      .map((row) => {
