...
async function getAuthorizedEvent(id: string, artisanId: string) {
  ...
  if (event.artisanId !== artisanId) return { status: 403 as const }
...
export async function GET(
  ...
  try {
    ...
    const result = await getAuthorizedEvent(id, session.artisanId)
    if (result.status === 404) return NextResponse.json({ success: false, error: 'Evenement introuvable' }, { status: 404 })
    if (result.status === 403) return NextResponse.json({ success: false, error: 'Acces non autorise' }, { status: 403 })
