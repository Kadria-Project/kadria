...
export async function GET(request: NextRequest) {
  try {
    ...
    const response:ClientListResponse={...paginated,summary:{totalClients:filtered.filter(i=>i.source==='canonical').length,activeClients:filtered.filter(i=>i.relation==='active').length,loyalClients:filtered.filter(i=>i.relation==='loyal').length,watchClients:filtered.filter(i=>i.relation==='watch').length,reactivationClients:filtered.filter(i=>i.relation==='reactivate').length,totalAcceptedValue:filtered.reduce((n,i)=>n+i.acceptedAmount,0),legacyEntries:filtered.filter(i=>i.source==='legacy').length,attentionCount:filtered.filter(i=>i.needsAttention).length},actions:{items:topClientActions(allActions),summary:summarizeClientActions(allActions)}}
