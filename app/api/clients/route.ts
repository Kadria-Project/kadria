...
export async function GET(request: NextRequest) {
  try {
    ...
    if(clientsResult.error) throw clientsResult.error; if(projectsResult.error) throw projectsResult.error
    ...
    const [quotesResult,appointmentsResult,activitiesResult,eventsResult]=ids.length?await Promise.all([
      supabase.from('Devis').select('project_id, total_ttc, total_ht, devis_amount, statut, accepted, accepted_at, sent_at, created_at, updated_at').in('project_id',ids),
      ...
      supabase.from('ProjectClientEvents').select('project_id, created_at, event_type').eq('tenant_id',context.tenantId).in('project_id',ids),
    ]):[{data:[],error:null},{data:[],error:null},{data:[],error:null},{data:[],error:null}]
    for(const result of [quotesResult,appointmentsResult,activitiesResult,eventsResult]) if(result.error) throw result.error
    ...
    const items=aggregateClientList({clients:asRows(clientsResult.data),projects,quotes,appointments,activities,events,includeArchived:request.nextUrl.searchParams.get('includeArchived')==='true'})
    ...
    const filtered=filterClientList(items,{q:params.get('q')||undefined,source:source==='canonical'||source==='legacy'?source:undefined,status:params.get('status')||undefined,active:booleanParam('active'),attention:booleanParam('attention'),recurring:booleanParam('recurring'),hasAppointment:booleanParam('hasAppointment')})
    const paginated=sortAndPaginateClientList(filtered,page,pageSize,['attention','lastInteraction','name','acceptedValue','projectCount','nextAppointment'].includes(sort||'') ? sort as import('@/src/lib/clients/client-list-aggregation').ClientListSort : 'attention',order==='asc'?'asc':'desc')
  ...
  } catch(error) { console.error('[CLIENTS_V2][LIST_FAILED]',{message:error instanceof Error?error.message:String(error)}); return NextResponse.json({success:false,error:'Impossible de charger les clients.'},{status:500}) }
}
