...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabaseAdmin.from(TABLES.projects).select('id, status, client_name, client_first_name, project_type, trade, budget, devis_amount, created_at, updated_at, quote_sent_at, accepted_at, last_follow_up_at', { count: 'exact' })
    ...
    query = query.order(sort === 'created' ? 'created_at' : sort === 'value' ? 'devis_amount' : 'updated_at', { ascending: false }).range((page - 1) * PAGE_SIZE, page * PAGE_SIZE - 1)
    ...
    return NextResponse.json({ success: true, items, total: count || 0, page, pageSize: PAGE_SIZE })
  } catch (error) {
