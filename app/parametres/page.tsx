...
function describeSettingsError(status: number, fallback: string): string {
  ...
  if (status === 403) return 'Vous n’avez pas accès à cette partie.'
  if (status === 404) return 'Cet élément n’est plus disponible.'
  if (status >= 500) return 'Kadria n’a pas pu terminer cette action. Réessayez dans un instant.'
  return fallback
}
...
function ParametresPageContent() {
  ...
  const webAssistantSettings: WebAssistantSettingsValues = {
    companyName: config.companyName,
  ...
  const updateWebAssistantSettings = (updater: (values: WebAssistantSettingsValues) => WebAssistantSettingsValues) => {
    setConfig((current) => ({
      ...current,
      ...updater({
        ...
        plan: current.plan,
        assistantAvatarType: current.assistantAvatarType,
        assistantAvatarUrl: current.assistantAvatarUrl,
        welcomeName: current.welcomeName,
        welcomeMessage: current.welcomeMessage,
        primaryColor: current.primaryColor,
        secondaryColor: current.secondaryColor,
        widgetColorMode: current.widgetColorMode,
        whiteLabelEnabled: current.whiteLabelEnabled,
        widgetBrandName: current.widgetBrandName,
        widgetBrandLogoUrl: current.widgetBrandLogoUrl,
      }),
    }))
  }
        ...
        <div className="flex shrink-0 items-center gap-3">
          {!isMobile && (
            <span style={{ color: saved ? '#4ade80' : 'var(--text-3)', fontSize: '12px', whiteSpace: 'nowrap' }}>
              {saved ? "Modifications enregistrées à l’instant" : saving ? 'Enregistrement en cours…' : canSaveActiveSection ? 'Réglages prêts à être enregistrés' : 'Section en lecture seule'}
            </span>
          )}
          <button
            type="button"
          ...
          >
            {!canSaveActiveSection ? 'Lecture seule' : saved ? 'Modifications enregistrées' : saving ? 'Enregistrement en cours…' : 'Enregistrer'}
          </button>
        </div>
      </div>
      ...
      <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-4 px-4 py-4 sm:px-6 sm:py-6 xl:px-10" style={{ alignItems: 'stretch' }}>
        ...
        <div className="min-w-0 w-full">
          ...
          {activeSection === 'widget' && (
            <WebAssistantSettingsSection
              values={webAssistantSettings}
              artisanId={artisanIdDisplay}
              isMobile={isMobile}
              canManage={canManageBusinessSettings}
              uploadingAvatar={uploadingTarget === 'assistant_avatar'}
              uploadingWhiteLabelLogo={uploadingTarget === 'white_label_logo'}
              uploadError={uploadError}
              showUploadError={uploadingTarget === null}
              onValuesChange={updateWebAssistantSettings}
              onUploadAssistantAvatar={handleAssistantAvatarFile}
              onUploadWhiteLabelLogo={handleWhiteLabelLogoFile}
              onGoToBilling={() => selectSection('offre')}
            />
          )}
