...
| Aperçu / PDF | **Non vérifié (desktop)** — la capture desktop censée montrer l'aperçu affiche en réalité une page d'erreur "Devis introuvable" (bug ou mauvais ID, cf. P1bis) ; le clic "Télécharger PDF" n'a pas ouvert de popup dans ce run (timeout 30s) ; seul l'aperçu **mobile** (état Envoyé) est confirmé visuellement |
...
- `devis-11-apercu-source-desktop.png` : **CORRECTION suite au contrôle visuel du lot
  de commit** — cette capture affiche en réalité l'écran **"Devis introuvable."**
  (route `/demo-dashboard/projet/[id]/devis/[devisId]` avec un `devisId` qui n'a pas
  résolu), pas la page détail devis attendue. La mention "OK" initialement inscrite
  ici était erronée — elle n'avait pas été vérifiée visuellement au moment de la
  rédaction initiale. Conservée dans ce commit **comme preuve d'un bug** (voir P1bis
  ci-dessous), pas comme preuve que l'aperçu fonctionne.
...
- `devis-19-modal-confirmation-desktop.png` : jamais produite dans ce lot (fichier
  absent du répertoire des captures) — capture manquante.
- Aperçu mobile : `devis-12-apercu-mobile.png` capturé avec succès (page détail devis
  responsive, lisible, état "Envoyé").

Statut : **Non vérifié** — aucune capture desktop valide de la page source de l'aperçu
n'a été obtenue dans ce lot (la seule tentative a atterri sur une page d'erreur "Devis
introuvable") ; le rendu PDF simulé n'a pas non plus pu être confirmé visuellement.
Seul l'aperçu mobile (état "Envoyé") a été confirmé visuellement.
...
5. **[P1bis] Capture "aperçu source" invalide — page "Devis introuvable"** — corrigé
   lors du contrôle visuel image par image effectué avant ce commit :
   `devis-11-apercu-source-desktop.png` avait été initialement décrite comme "OK"
   dans une version antérieure de ce document sans vérification visuelle réelle. En
   l'ouvrant réellement, la capture montre l'écran d'erreur **"Devis introuvable."**
   Cause non investiguée dans ce lot (ID de devis erroné dans le script d'audit,
   navigation avant chargement complet des données démo, ou vrai défaut de résolution
   de route) — à re-tester. Conservée dans le commit comme preuve du problème, pas
   comme preuve que l'aperçu fonctionne. Rappel méthodologique : ceci illustre
   pourquoi le nom de fichier seul ne doit jamais être considéré comme une preuve
   suffisante — seul le contenu visuel réellement observé fait foi.
6. **[P3] `devis-08-catalogue-desktop.png` mal nommée** — le nom suggère un
   sélecteur de catalogue de prestations, mais le contenu réel est le haut du
   formulaire de création (identique à `devis-06`/`devis-09` avec un total de
   738,00 € déjà calculé). Aucun écran de "catalogue" dédié n'a été localisé dans ce
   lot. Conservée car elle reste une preuve valide du formulaire de création rempli,
   mais le nom est trompeur.

## Nettoyage (lot de commit du 2026-07-12)

Contrôle visuel image par image effectué sur les 22 PNG candidats de la zone Devis
avant tout commit. Résultat :

- **19 captures validées** comme preuves cohérentes de leur scénario annoncé.
- **1 capture conservée comme preuve d'un bug** malgré un contenu inattendu
  (`devis-11-apercu-source-desktop.png` → "Devis introuvable", voir P1bis).
- **3 captures rejetées et non committées** :
  - `desktop-devis-list.png` (ancienne capture, doublon obsolète superseded par
    `devis-01/02/03-liste-*-desktop.png`)
  - `devis-04-liste-mobile.png` (a capturé le bottom-sheet "Menu Kadria" au lieu de
    la liste — sélecteur de nav obsolète)
  - `devis-19-modal-confirmation-desktop.png` (jamais produite — absente du
    répertoire)
- `_run-log.txt` (log Playwright temporaire, aucune donnée sensible détectée) et les
  scripts `audit_devis3.js`/`audit_devis_full.js` (déjà absents de l'arbre de travail
  au moment du commit) sont des artefacts de travail non committés.
- Des captures d'autres zones d'audit (`desktop-*.png`, `mobile-*.png` génériques,
  `dossiers-02-liste-dense-desktop.png`) sont présentes non suivies dans le répertoire
  mais **hors périmètre de ce lot** (zone Dashboard/nav générale, pas Devis) — non
  committées, non supprimées, laissées en l'état pour un lot ultérieur dédié.

## Build

`npm run build` exécuté avant commit → voir résultat consigné dans le rapport de la
tâche de commit. Aucune modification de code produit dans ce lot — uniquement des
captures et de la documentation.
...
pas la réédition d'un brouillon existant pré-rempli), mobile (création confirmée,
aperçu mobile confirmé, liste mobile capturée par erreur — mauvais écran).
Non vérifié : aperçu/PDF desktop (la capture prévue montre une erreur "Devis
introuvable" au lieu de la page source — voir bug P1bis ; rendu PDF simulé jamais
confirmé, aucune popup ouverte).

Aucun appel métier réel détecté (Stripe/Resend/email/SMS/écriture Supabase) sur
l'ensemble du parcours testé initialement documenté. Seules les preuves visuelles
(captures + documentation) sont committées et poussées sur `main` dans ce lot ; aucune
logique produit n'a été modifiée.
