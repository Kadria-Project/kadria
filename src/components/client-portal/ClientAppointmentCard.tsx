...
type Props = { appointment: ClientPortalAppointment; onSubmit: (input: { status: 'confirmed' | 'change_requested' | 'cancelled'; note: string; requestId: string; expectedVersion: number }) => Promise<void> }
...
export default function ClientAppointmentCard({ appointment, onSubmit }: Props) {
  const [action, setAction] = useState<'confirmed' | 'change_requested' | 'cancelled' | null>(null)
  const [note, setNote] = useState('')
  const [saving, setSaving] = useState(false)
  const submit = async () => { if (!action || saving) return; setSaving(true); try { await onSubmit({ status: action, note, requestId: crypto.randomUUID(), expectedVersion: appointment.version }); setAction(null); setNote('') } finally { setSaving(false) } }
  ...
  return <section style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 1px 3px rgba(0,0,0,.06)', marginBottom: 20 }} aria-live="polite">
    <h2 style={{ fontSize: 15, margin: '0 0 12px' }}>Votre rendez-vous</h2>
    <span style={{ display: 'inline-block', padding: '6px 10px', borderRadius: 999, background: appointment.status === 'confirmed' ? '#dcfce7' : appointment.status === 'cancelled' ? '#fee2e2' : '#fef3c7', fontSize: 12, fontWeight: 700 }}>{labels[appointment.status]}</span>
    ...
    {appointment.status === 'change_requested' && <p style={{ color: '#92400e', fontSize: 13 }}>Votre demande de changement a bien été transmise.</p>}
    {appointment.status === 'cancelled' && <p style={{ color: '#b91c1c', fontSize: 13 }}>Ce rendez-vous est annulé.</p>}
    {hasActions && !action && <div style={{ display: 'grid', gap: 10, marginTop: 18 }}><button type="button" onClick={() => setAction('confirmed')} style={{ border: 0, borderRadius: 8, padding: 12, background: '#16a34a', color: '#fff', fontWeight: 700 }}>Confirmer le rendez-vous</button><button type="button" onClick={() => setAction('change_requested')} style={{ border: '1px solid #d1d5db', borderRadius: 8, padding: 12, background: '#fff', fontWeight: 700 }}>Demander un changement</button><button type="button" onClick={() => setAction('cancelled')} style={{ border: 0, background: 'transparent', color: '#991b1b', padding: 6 }}>Refuser le rendez-vous</button></div>}
