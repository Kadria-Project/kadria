...
export function IntegrationSettingsView() {
  ...
  if (!data.permissions.canRead && !data.error) return <ReadOnlyNotice message="Vous n’avez pas accès aux connexions." />

  return <div className="space-y-4">
    {data.error && <div role="alert" className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><p>{data.error}</p><button type="button" onClick={() => void data.reload()} className="mt-3 rounded-lg border border-amber-300 bg-white px-3 py-2 font-medium">Réessayer</button></div>}
    <SettingsPanel title="Applications connectées" description="Choisissez les outils auxquels Kadria peut accéder pour vous aider au quotidien.">
      <article className="rounded-lg border border-slate-200 p-4">
        ...
        {!data.permissions.canManage && <ReadOnlyNotice message="La gestion des connexions est réservée au propriétaire et aux administrateurs." />}
        {data.permissions.canManage && <div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={data.connect} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">{data.calendar.connected ? 'Reconnecter Google Calendar' : 'Connecter Google Calendar'}</button>{data.calendar.connected && <button type="button" onClick={() => void data.disconnect()} disabled={data.action === 'disconnecting'} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 disabled:opacity-60">{data.action === 'disconnecting' ? 'Déconnexion…' : 'Déconnecter'}</button>}</div>}
      </article>
    </SettingsPanel>
