...
export type ExecutiveSummaryProps = { firstName: string | null; kpis: KPIResult[]; analytics: PerformanceAnalytics | null }

export function formatGreeting(firstName: string | null | undefined): string {
  const value = typeof firstName === 'string' ? firstName.trim() : ''
  if (!value || value.includes('@')) return 'Bonjour.'
  return `Bonjour ${value.charAt(0).toLocaleUpperCase('fr-FR')}${value.slice(1).toLocaleLowerCase('fr-FR')}.`
}

export default function ExecutiveSummary({ firstName, kpis, analytics }: ExecutiveSummaryProps) {
