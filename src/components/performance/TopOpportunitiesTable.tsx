...
function ValueCell({ value }: { value: PerformanceOpportunity['value'] }) {
  ...
  return (
    <span title={value.label} className="whitespace-nowrap text-sm font-semibold text-slate-950">
      {formatKPIValue(value.amount, 'currency')}
      <span className="ml-1 hidden text-[11px] font-normal text-slate-400 2xl:inline">({value.label})</span>
    </span>
  )
}
...
function OpportunityRow({ opportunity, index }: { opportunity: PerformanceOpportunity; index: number }) {
  return (
    <tr className="border-b border-slate-100 last:border-0 hover:bg-emerald-50/40">
      ...
      <td className="max-w-[180px] px-2 py-3.5 align-top text-sm text-slate-600" title={opportunity.projectTitle}><span className="block truncate">{opportunity.projectTitle}</span></td>
      ...
      <td className="py-3.5 pr-5 align-top text-right">
        <Link
          ...
          className="inline-flex whitespace-nowrap items-center rounded-lg border border-slate-200 px-2 py-1.5 text-[11px] font-semibold text-slate-700 hover:border-emerald-300 hover:text-emerald-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-500"
        >
          Ouvrir le dossier
        </Link>
      </td>
      ...
      <div className="hidden md:block overflow-x-auto">
          ...
          <thead>
            <tr className="border-b border-slate-200 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
              <th scope="col" className="w-[15%] py-2 pl-4 pr-2">Client</th>
              <th scope="col" className="w-[24%] py-2 px-2">Projet</th>
              <th scope="col" className="w-[14%] py-2 px-2">Statut</th>
              <th scope="col" className="w-[13%] py-2 px-2">Valeur</th>
              <th scope="col" className="hidden 2xl:table-cell py-2 px-2">Score</th>
              <th scope="col" className="w-[18%] py-2 px-2">Prochaine action</th>
              <th scope="col" className="hidden 2xl:table-cell py-2 px-2">Échéance</th>
              <th scope="col" className="w-[16%] py-2 pr-4 text-right">Action</th>
            </tr>
          </thead>
          <tbody>
