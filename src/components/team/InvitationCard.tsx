      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-sm text-zinc-400">
            {name} - {TEAM_ROLE_LABELS[invitation.role]} - {invitation.jobTitle || 'Fonction non renseignee'}
          </p>
          <p className="mt-2 text-xs text-zinc-500">
            Invitation envoyee par {invitation.invitedByName || 'Kadria'}
            {invitation.expiresAt ? ` - expire le ${new Date(invitation.expiresAt).toLocaleString('fr-FR')}` : ''}
            {` - relance(s) : ${Math.max(invitation.sendCount - 1, 0)}`}
          </p>
        </div>
