...
function PriorityCard({ item, index }: { item: HomeBriefItem; index: number }) {
  ...
  return <article className={`flex h-full flex-col rounded-2xl border-l-[3px] p-4 shadow-[0_4px_18px_rgba(15,34,50,.035)] ${style.accent}`}>
    ...
    <div className="mt-auto pt-4"><Link href={item.action.href} className={`inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg px-3 py-2 text-[11px] font-bold transition-colors ${style.cta}`}>{item.action.label}<ArrowRight className="size-3.5" /></Link></div>
  </article>
}
...
function Agenda({ brief }: { brief: HomeBrief }) {
  ...
  return <section aria-labelledby="home-agenda" className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_4px_18px_rgba(15,34,50,.025)]">
    ...
    {events.length ? <div className="mt-3 overflow-x-auto pb-1"><div className="relative grid min-w-[720px] grid-flow-col auto-cols-[minmax(210px,1fr)] gap-5"><span aria-hidden="true" className="absolute left-0 right-0 top-[21px] h-px bg-emerald-200" />{events.map((event) => <article key={event.id} className="relative min-w-0"><div className="flex items-center gap-2"><span className="rounded-md bg-emerald-50 px-1.5 py-1 text-[9px] font-bold uppercase tracking-wide text-emerald-700">{event.day === 'today' ? 'Aujourd’hui' : 'Demain'}</span><span className="text-xs font-bold text-slate-900">{event.time}</span></div><span className="absolute left-0 top-[18px] size-2 rounded-full bg-emerald-500 ring-4 ring-white" /><div className="mt-3 border-l-2 border-emerald-100 pl-3"><div className="flex flex-wrap items-center gap-1.5"><p className="text-xs font-bold text-slate-800">{event.title}</p><span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-500">Rendez-vous</span></div><p className="mt-1 text-[11px] text-slate-500">{event.context}</p>{event.location ? <p className="mt-1 flex items-center gap-1 text-[10px] text-slate-400"><MapPin className="size-3" />{event.location}</p> : null}</div></article>)}</div></div> : <p className="mt-4 text-xs text-slate-500">Aucun rendez-vous prévu aujourd’hui ou demain.</p>}
  </section>
}
