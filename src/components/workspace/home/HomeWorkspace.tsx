...
export default function HomeWorkspace({ firstName, todayLabel, projects, todayEvents: rawEvents, operationsCenter, onOpenProject, onOpenAgenda }: Props) {
  ...
  return <div className="space-y-7 pb-8">
    <section className="rounded-2xl border border-slate-200 bg-white px-6 py-5"><div className="flex items-end justify-between gap-6"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{todayLabel}</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-[#0b2232]">Bonjour{firstName ? ` ${firstName}` : ''}.</h2><p className="mt-1 max-w-2xl text-base leading-6 text-slate-600">{summary}</p></div><div className="hidden shrink-0 gap-4 text-right text-xs text-slate-500 2xl:flex"><span>{activity.length} activité{activity.length > 1 ? 's' : ''}</span><span>{events.length} rendez-vous</span><span>{week}</span></div></div></section>
    <section><h3 className="text-lg font-semibold text-[#0b2232]">Décisions à prendre</h3>{focus.length ? <div className="mt-4 grid gap-3 md:grid-cols-2">{focus.map((item, index) => <article key={item.id} className={`rounded-2xl border border-slate-200 bg-white p-4 ${index === 0 ? 'md:col-span-2 md:p-5' : ''}`}><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-700">{index === 0 ? 'Décision principale' : 'À garder en vue'}</p><p className={`mt-2 font-semibold text-[#0b2232] ${index === 0 ? 'text-xl' : 'text-base'}`}>{item.title}</p><p className="mt-1 text-sm text-slate-600">{item.description}</p><p className="mt-3 text-sm text-slate-500"><span className="font-medium text-slate-700">Pourquoi :</span> {item.reason}</p>{index === 0 && <p className="mt-1 text-sm text-slate-500"><span className="font-medium text-slate-700">Prochaine décision :</span> {item.primaryActionLabel || 'Ouvrir le dossier'}</p>}</div>{item.projectId && <button type="button" onClick={() => onOpenProject(item.projectId!)} className="shrink-0 text-sm font-semibold text-emerald-700 hover:text-emerald-600">Ouvrir →</button>}</div></article>)}</div> : <div className="mt-4 rounded-2xl bg-emerald-50 p-5"><p className="font-semibold text-emerald-950">Tout est sous contrôle.</p><p className="mt-1 text-sm text-emerald-900/75">Aucune action urgente aujourd’hui. Vous pouvez vous concentrer sur vos interventions.</p></div>}</section>
