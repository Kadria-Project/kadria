...
function CanWait({ brief }: { brief: HomeBrief }) {
  ...
  return <section aria-labelledby="home-can-wait" className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-[0_4px_18px_rgba(15,34,50,.025)]">
    ...
    {deferred.length ? <div className="mt-1.5 divide-y divide-slate-100">{deferred.map((item) => <Link key={item.id} href={item.action.href} className="flex items-center gap-2.5 py-1.5 text-xs hover:text-emerald-700"><span className="grid size-6 shrink-0 place-items-center rounded-lg bg-slate-50 text-slate-500">{iconFor(item, 'size-3.5')}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold text-slate-800">{item.title}</span><span className="block truncate text-[11px] text-slate-500">{item.context ? `${item.context.clientName} · ${item.context.projectTitle}` : item.observation}</span></span><span className="shrink-0 text-[11px] text-slate-400">{item.recommendation}</span></Link>)}</div> : <p className="mt-2 text-xs leading-5 text-slate-500">{brief.canWait}</p>}
  </section>
}
...
function CompactProduction({ performance }: { performance: { kpis: KPIResult[]; analytics: PerformanceAnalytics } }) {
  ...
  return <div className="mt-3 border-t border-slate-100 pt-3">
    ...
    <div className="mt-2 h-24" role="img" aria-label={`Évolution du CA signé : ${series.points.map((point) => `${point.label} ${formatKPIValue(point.revenue, 'currency')}`).join(', ')}`}>
...
function ActivitySummary({ brief, performance }: { brief: HomeBrief; performance: Props['performance'] }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-[0_4px_18px_rgba(15,34,50,.025)]"><h2 className="text-[10px] font-bold uppercase tracking-[.14em] text-slate-500">Résumé de votre activité</h2><div className="mt-2.5 grid grid-cols-2 gap-2 sm:grid-cols-4">{metrics.map(([label, value]) => <div key={label} className="rounded-xl border border-slate-100 bg-slate-50/70 px-2.5 py-2"><p className="text-[10px] leading-4 text-slate-500">{label}</p><p className="mt-0.5 text-lg font-bold tracking-tight text-slate-950">{value}</p></div>)}</div>{performance ? <CompactProduction performance={performance} /> : null}</section>
}
...
export default function HomeWorkspace({ firstName, brief, loadState, onRefresh, performance }: Props) {
  ...
  return <div className="mx-auto max-w-[1320px] space-y-4 pb-4">
    ...
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.95fr)_minmax(300px,.95fr)]"><section aria-labelledby="home-priorities"><div className="mb-2.5 flex items-center gap-2"><Sparkles className="size-4 text-emerald-600" /><h2 id="home-priorities" className="text-[10px] font-bold uppercase tracking-[.14em] text-slate-600">Vos 3 priorités du jour</h2></div>{priorities.length ? <div className="space-y-1.5">{priorities.map((item, index) => <PriorityCard key={item.id} item={item} index={index} />)}</div> : <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-sm text-emerald-900">Votre journée est sous contrôle. Aucun dossier ne demande une décision immédiate.</div>}</section><Agenda brief={brief} /></div>
    <div className="grid items-start gap-4 lg:grid-cols-[minmax(280px,.8fr)_minmax(0,1.2fr)]"><CanWait brief={brief} /><ActivitySummary brief={brief} performance={performance} /></div>
