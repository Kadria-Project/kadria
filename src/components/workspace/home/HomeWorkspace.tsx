...
function ActivitySummary({ brief }: { brief: HomeBrief }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-[0_4px_18px_rgba(15,34,50,.025)]"><div className="flex items-center justify-between gap-3"><h2 className="text-[10px] font-bold uppercase tracking-[.14em] text-slate-500">Résumé de votre activité</h2><span className="text-[10px] font-semibold text-slate-400">Données disponibles</span></div><div className="mt-2.5 grid grid-cols-2 gap-2 sm:grid-cols-4">{metrics.map(([label, value]) => <div key={label} className="rounded-xl border border-slate-100 bg-slate-50/70 px-2.5 py-2"><p className="text-[10px] leading-4 text-slate-500">{label}</p><p className="mt-0.5 text-lg font-bold tracking-tight text-slate-950">{value}</p></div>)}</div></section>
}
...
export default function HomeWorkspace({ firstName, brief, loadState, onRefresh }: Props) {
  ...
  return <div className="mx-auto max-w-[1320px] space-y-4 pb-4">
    ...
    <div className="grid items-start gap-4 lg:grid-cols-[minmax(280px,.8fr)_minmax(0,1.2fr)]"><CanWait brief={brief} /><ActivitySummary brief={brief} /></div>
