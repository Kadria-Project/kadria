    ...
    <article className={`rounded-2xl border ${primary ? 'border-orange-200 bg-[#fffaf0] p-4' : 'border-slate-200 bg-white px-4 py-3.5'}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.13em] ${primary ? 'text-orange-700' : 'text-slate-500'}`}>{primary ? <Flame aria-hidden="true" className="size-4 text-orange-500" /> : <Eye aria-hidden="true" className="size-4 text-slate-500" />}{primary ? 'À traiter en premier' : 'À garder en vue'}</p>
          <h3 className={`mt-1.5 font-semibold text-[#0b2232] ${primary ? 'text-xl' : 'text-base'}`}>{item.title}</h3>
          <p className="mt-1 text-sm leading-[1.2rem] text-slate-600">{item.description}</p>
