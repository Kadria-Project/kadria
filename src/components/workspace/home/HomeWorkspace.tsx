...
export default function HomeWorkspace({ firstName, todayLabel, projects, todayEvents: rawEvents, operationsCenter, onOpenProject, onOpenAgenda }: Props) {
  ...
  const summary = focus.length > 0 ? `${focus.length} sujet${focus.length > 1 ? 's méritent' : ' mérite'} votre attention aujourd’hui.` : events.length > 0 ? `La journée est calme. ${events.length} rendez-vous ${events.length > 1 ? 'sont prévus' : 'est prévu'} et aucune urgence n’est détectée.` : 'Tout est sous contrôle aujourd’hui.';
  ...
  return <div className="space-y-10 pb-10">
    ...
    <section><h3 className="text-lg font-semibold text-[#0b2232]">Priorités du jour</h3>{focus.length ? <div className="mt-4 space-y-3">{focus.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-start justify-between gap-4"><div><p className="font-semibold text-[#0b2232]">{item.title}</p><p className="mt-1 text-sm text-slate-600">{item.description}</p><p className="mt-2 text-sm text-slate-500">Pourquoi : {item.reason}</p></div>{item.projectId && <button type="button" onClick={() => onOpenProject(item.projectId!)} className="shrink-0 rounded-xl bg-emerald-500 px-3 py-2 text-sm font-semibold text-emerald-950 hover:bg-emerald-400">Ouvrir le dossier</button>}</div></article>)}</div> : <p className="mt-2 text-slate-600">Tout est sous contrôle aujourd’hui.</p>}</section>
    ...
    {watchlist.length > 0 && <section><h3 className="text-lg font-semibold text-[#0b2232]">Points à surveiller</h3><div className="mt-3 space-y-2">{watchlist.map((item) => <div key={item.id} className="border-l-2 border-amber-400 pl-4"><p className="font-medium text-slate-800">{item.title}</p><p className="mt-1 text-sm text-slate-600">{item.reason}</p></div>)}</div></section>}
  </div>;
}
