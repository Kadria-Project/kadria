...
export default function QuickCreate() {
  ...
  return <div role="dialog" aria-modal="true" aria-label="Créer" className="fixed inset-0 flex items-end bg-slate-950/30 sm:items-start sm:justify-end sm:p-6" style={{ zIndex: SHELL_OVERLAY_LAYERS.popover }} onMouseDown={(event) => { if (event.target === event.currentTarget) closeQuickCreate() }}>
    <div className="w-full rounded-t-2xl bg-white p-3 shadow-2xl sm:w-80 sm:rounded-2xl" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {actions.length ? actions.map((action) => <button key={action.id} type="button" onClick={() => { closeQuickCreate(); router.push(action.href) }} className="flex min-h-12 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-medium text-slate-800 hover:bg-emerald-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-500"><Plus className="size-4 text-emerald-700" />{action.label}</button>) : <p className="px-3 py-5 text-sm text-slate-500">Aucune action disponible.</p>}
