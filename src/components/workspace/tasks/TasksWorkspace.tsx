...
function SituationCard({ situation, onAction, busy }: { situation: WorkSituation; onAction: (action: WorkSituationAction) => void; busy: boolean }) {
  ...
  return (
    <article id={`workspace-action-${situation.id}`} className={`group relative overflow-hidden rounded-[18px] border border-slate-200/80 bg-white px-4 py-3.5 shadow-[0_3px_12px_rgba(15,34,50,0.035)] transition-[transform,border-color,box-shadow] duration-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_10px_24px_rgba(15,34,50,0.06)] before:absolute before:inset-y-0 before:left-0 before:w-[3px] ${accent}`}>
      <div className="grid items-center gap-4 xl:grid-cols-[minmax(230px,0.85fr)_minmax(340px,1.45fr)_minmax(185px,0.55fr)]">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">{situation.observedFacts.join(' ')}</p>
          </div>
        </div>

        <div className="min-w-0 space-y-1.5 text-xs leading-5 text-slate-600">
          <p className="flex gap-2"><Lightbulb className="mt-0.5 size-3.5 shrink-0 text-amber-500" aria-hidden="true" /><span><span className="font-semibold text-slate-800">Ce que cela signifie :</span> {situation.understanding}</span></p>
          {situation.recommendation && <p className="flex gap-2 text-emerald-950"><ShieldCheck className="mt-0.5 size-3.5 shrink-0 text-emerald-600" aria-hidden="true" /><span><span className="font-semibold">Recommandation :</span> {situation.recommendation}</span></p>}
          <details className="text-[11px] text-slate-500"><summary className="cursor-pointer font-medium text-slate-500">Pourquoi cela compte</summary><p className="mt-1">{situation.importance}{situation.consequence ? ` ${situation.consequence}` : ''}</p></details>
        </div>

        <div className="flex min-w-0 flex-row items-center gap-2 xl:flex-col xl:items-stretch">
          <span className="hidden self-start rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-semibold text-emerald-700 xl:inline-flex">Niveau de preuve : {proofLabel(situation.confidence)}</span>
...
function SecondaryLane({ lane, situations, onShowAll, onAction, busyAction }: { lane: WorkLane; situations: WorkSituation[]; onShowAll: () => void; onAction: (situation: WorkSituation) => void; busyAction: string | null }) {
  ...
  return (
    <section className="min-w-0 rounded-[16px] border border-slate-200/80 bg-white px-3.5 py-3 shadow-[0_2px_8px_rgba(15,34,50,0.025)]">
      ...
      <div className="mt-2 divide-y divide-slate-100">
        {situations.length ? situations.slice(0, 3).map((situation) => <button key={situation.id} type="button" disabled={busyAction === situation.id} onClick={() => onAction(situation)} className="flex w-full items-center gap-2 py-2 text-left transition-colors hover:text-emerald-700 disabled:opacity-60"><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold text-[#0b2232]">{contextLabel(situation)}</span><span className="mt-0.5 block truncate text-[11px] text-slate-500">{situation.primaryAction?.label || situation.observedFacts[0]}</span></span><ArrowRight className="size-3.5 shrink-0 text-slate-400" aria-hidden /></button>) : <p className="py-3 text-[11px] leading-4 text-slate-400">{meta.empty}</p>}
      </div>
      {situations.length > 0 && <button type="button" onClick={onShowAll} className="mt-2 inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 hover:text-emerald-800">Voir tout <ArrowRight className="size-3" aria-hidden /></button>}
    </section>
  )
}
  ...
  return (
    <div className="w-full max-w-none space-y-3.5 pb-4">
      ...
      {state.kind === 'active' && <>
        ...
        <section aria-label="Actions secondaires" className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
          {(Object.keys(laneMeta).filter((lane) => lane !== 'decide') as WorkLane[]).map((lane) => <SecondaryLane key={lane} lane={lane} situations={lanes[lane]} busyAction={busyAction} onShowAll={() => setView(lane)} onAction={(situation) => situation.primaryAction && void handleAction(situation, situation.primaryAction)} />)}
        </section>
      </>}
