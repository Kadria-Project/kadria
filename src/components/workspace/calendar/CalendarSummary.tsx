...
export default function CalendarSummary({ appointmentCount, plannedMinutes, conflictCount, travelWarningCount }: CalendarSummaryProps) {
  ...
  return <section aria-label="Résumé de la journée" className="grid grid-cols-2 gap-3 xl:grid-cols-4">{metrics.map(({ label, value, note, Icon, tone }) => <div key={label} className="relative flex min-h-[92px] items-center gap-3 overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.025)]"><span className={`absolute inset-x-0 top-0 h-0.5 ${tone.split(' ')[0]}`} /><span className={`grid size-10 shrink-0 place-items-center rounded-xl ${tone}`}><Icon className="size-5" aria-hidden="true" /></span><div><p className="text-lg font-semibold tracking-tight text-slate-950">{value}</p><p className="text-xs font-medium text-slate-700">{label}</p><p className="mt-1 text-[11px] text-slate-500">{note}</p></div></div>)}</section>;
}
