...
export default function CalendarWorkspace() {
  ...
  const events = useMemo(
    () => mode === 'google' ? googleEvents.map(normalizeGoogleEvent) : appointments.map(normalizeKadriaAppointment),
    [appointments, googleEvents, mode],
  );
  const todayEvents = useMemo(
    () => events.filter((event) => { const date = eventDate(event); return date ? isSameDay(date, new Date()) : false; }),
    [events],
  );
  ...
  const nextAppointment = useMemo(
    () => events.filter((event) => { const date = eventDate(event); return date && date.getTime() >= currentTime; }).sort((left, right) => (eventDate(left)?.getTime() || 0) - (eventDate(right)?.getTime() || 0))[0] || null,
    [currentTime, events],
  );
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await fetch('/api/appointments/create', {
        ...
        body: JSON.stringify({ title: form.title, start: new Date(form.start).toISOString(), end: form.end ? new Date(form.end).toISOString() : undefined, location: form.location || undefined, eventType: 'appointment' }),
      });
      const json = await response.json();
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-4 pb-6">
      ...
      {createOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/30 p-4" onClick={() => setCreateOpen(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl" onClick={(event) => event.stopPropagation()}>
