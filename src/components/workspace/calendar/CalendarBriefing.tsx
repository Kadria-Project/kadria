...
export default function CalendarBriefing({ appointmentCount, conflictCount, availableMinutes, view, onToggleView, onCreate }: CalendarBriefingProps) {
  ...
  const detail = hasConflict
    ...
    : appointmentCount
      ? `${appointmentCount} rendez-vous prévus aujourd’hui.${availableMinutes > 0 ? ` ${Math.floor(availableMinutes / 60)} h ${availableMinutes % 60} restent non planifiées.` : ''}`
      : `Aucun rendez-vous n’est prévu aujourd’hui. ${Math.floor(availableMinutes / 60)} h restent non planifiées.`;
    ...
    <section id="workspace-section-briefing" className="rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-[0_1px_2px_rgba(15,23,42,0.03)] sm:px-6">
      ...
      <div className="mt-2 flex flex-wrap items-center justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div>
            <h2 className="text-xl font-semibold tracking-tight text-slate-950">{isLight && !hasConflict ? 'Votre journée est calme.' : title}</h2>
