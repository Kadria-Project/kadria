  ...
  return (
    <div className="group absolute z-10" style={{ top, height, left, width }} aria-busy={saving}>
      <button
        ...
        <span className="block truncate text-[10px] text-current/70">{event.clientName || event.projectTitle || compactAssigneeName(event) || 'Rendez-vous'}</span>
      </button>
        ...
        <div className={['grid min-w-[760px]', view === 'jour' ? 'grid-cols-[56px_minmax(0,1fr)]' : 'grid-cols-[56px_repeat(7,minmax(120px,1fr))]'].join(' ')}>
          ...
          {days.map((day) => {
            ...
            return <div key={key} ref={(node) => { if (node) dayColumnsRef.current.set(key, node); else dayColumnsRef.current.delete(key); }} tabIndex={0} aria-label={`Planning du ${day.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}. Appuyez sur Entrée pour ajouter un rendez-vous.`} onKeyDown={(keyboardEvent) => { if (keyboardEvent.key !== 'Enter' && keyboardEvent.key !== ' ') return; keyboardEvent.preventDefault(); const suggested = suggestedStartForDay(day); onCreate(day, undefined, suggested); }} onMouseLeave={() => setHoveredSlot(null)} onMouseMove={(mouseEvent) => { if (dragging) return; const bounds = mouseEvent.currentTarget.getBoundingClientRect(); const offset = Math.max(0, Math.min(gridHeight - 1, mouseEvent.clientY - bounds.top)); setHoveredSlot({ key, minutes: range.startMinutes + snapMinutes((offset / CALENDAR_HOUR_HEIGHT) * 60) }); }} onDragOver={(dragEvent) => { if (dragging) dragEvent.preventDefault(); }} onDrop={(dragEvent) => { dragEvent.preventDefault(); if (!dragging) return; const bounds = dragEvent.currentTarget.getBoundingClientRect(); const offset = Math.max(0, Math.min(gridHeight - 1, dragEvent.clientY - bounds.top)); const minutes = range.startMinutes + snapMinutes((offset / CALENDAR_HOUR_HEIGHT) * 60); const start = new Date(day); start.setHours(Math.floor(minutes / 60), minutes % 60, 0, 0); onMoveEvent(dragging, start); setDragging(null); setInteractionRange(null); }} className={['group relative border-r border-[#DCE5E2] focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-500', isToday ? 'bg-[#F7FCF9]' : 'bg-white'].join(' ')} style={{ height: gridHeight }}>
              ...
              {hoveredStart && !occupiedHover ? <button type="button" onClick={(event) => { event.stopPropagation(); onCreate(day, event.currentTarget, hoveredStart); }} className="absolute right-2 z-30 grid size-7 place-items-center rounded-full bg-emerald-500 text-white opacity-0 shadow-sm transition-opacity group-hover:opacity-100 hover:bg-emerald-600 focus:opacity-100 focus:outline focus:outline-2 focus:outline-emerald-600 focus:outline-offset-2" style={{ top: Math.max(4, (((hovered?.minutes || range.startMinutes) - range.startMinutes) / 60) * CALENDAR_HOUR_HEIGHT + 4) }} aria-label={`Ajouter un rendez-vous le ${day.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric' })} à ${String(Math.floor((hovered?.minutes || range.startMinutes) / 60)).padStart(2, '0')} h ${String((hovered?.minutes || range.startMinutes) % 60).padStart(2, '0')}`}><Plus className="size-4" /></button> : null}
