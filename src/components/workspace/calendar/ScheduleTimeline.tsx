...
type HoveredSlot = { key: string; minutes: number };
...
function TimelineEvent({ event, range, currentTime, placement, displayColumns, highlighted, onOpen, qualificationAvailable, saving, onDragStart, onDragEnd, onResizeStart, onResizeKeyDown }: {
  ...
  const compact = height < 72 || displayColumns > 2;
  ...
  return (
    <div className="group absolute z-10" style={{ top, height, left, width }} aria-busy={saving}>
      <button
        ...
        >
          {event.assignedUserName ? <span aria-label={`Responsable : ${event.assignedUserName}`} title={event.assignedUserName} className="absolute right-1.5 top-1.5 grid size-5 place-items-center rounded-full bg-white/85 text-[8px] font-bold text-slate-700">{getInitials(event.assignedUserName)}</span> : null}
        ...
        <span title={event.title} className="block truncate text-[11px] font-bold leading-4">{event.title}</span>
        <span title={event.clientName || event.projectTitle || compactAssigneeName(event) || 'Rendez-vous'} className="block truncate text-[10px] text-current/70">{event.clientName || event.projectTitle || compactAssigneeName(event) || 'Rendez-vous'}</span>
        {!compact ? <span className="mt-1 flex flex-wrap gap-1">{event.source === 'kadria-appointment' ? <span className={['inline-flex rounded-full px-1.5 py-0.5 text-[9px] font-semibold', statusPresentation.badgeClassName].join(' ')}>{statusPresentation.label}</span> : null}{qualificationLabel ? <span className="inline-flex rounded-full bg-white/75 px-1.5 py-0.5 text-[9px] font-semibold text-current/75">{qualificationLabel}</span> : null}</span> : null}
      </button>
...
export default function ScheduleTimeline({ view, selectedDate, events, onPrevious, onNext, onToday, onViewChange, onOpenEvent, onCreate, qualificationAvailable, workStartTime, workEndTime, savingEventIds, onMoveEvent, onResizeEvent }: ScheduleTimelineProps) {
  ...
  return (
    <section id="workspace-section-calendar" className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.025)] sm:p-5 lg:min-h-[70vh]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2"><p className="mr-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500"><CalendarDays className="size-4 text-emerald-600" />Planning</p><button type="button" onClick={onPrevious} aria-label={`${periodLabel} précédent`} className="grid size-8 place-items-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50"><ChevronLeft className="size-4" /></button><button type="button" onClick={onToday} aria-label="Revenir à aujourd’hui" disabled={selectedDateIsToday} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-default disabled:border-slate-100 disabled:bg-slate-50 disabled:text-slate-400">Aujourd’hui</button><p aria-live="polite" className="min-w-[13rem] text-center text-sm font-semibold text-slate-800">{selectedDateLabel}</p><button type="button" onClick={onNext} aria-label={`${periodLabel} suivant`} className="grid size-8 place-items-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50"><ChevronRight className="size-4" /></button>{view === 'jour' && selectedDateIsToday ? <button type="button" onClick={handleNow} aria-label="Aller à l’heure actuelle" className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">Maintenant</button> : null}</div>
      ...
      <div ref={scrollRef} className="mt-4 overflow-x-auto overflow-y-visible pb-1">
        <div className={['grid min-w-[760px]', view === 'jour' ? 'grid-cols-[56px_minmax(0,1fr)]' : 'grid-cols-[56px_repeat(7,minmax(120px,1fr))]'].join(' ')}>
          ...
          {days.map((day) => {
            ...
            const hovered = hoveredSlot?.key === key ? hoveredSlot : null;
            const hoveredStart = hovered ? new Date(day.getFullYear(), day.getMonth(), day.getDate(), Math.floor(hovered.minutes / 60), hovered.minutes % 60) : null;
            const occupiedHover = hoveredStart && dayEvents.some((event) => { const start = eventDate(event); const end = event.end ? new Date(event.end) : null; return start && end && hoveredStart >= start && hoveredStart < end; });
            return <div key={key} ref={(node) => { if (node) dayColumnsRef.current.set(key, node); else dayColumnsRef.current.delete(key); }} tabIndex={0} aria-label={`Planning du ${day.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}. Appuyez sur Entrée pour ajouter un rendez-vous.`} onKeyDown={(keyboardEvent) => { if (keyboardEvent.key !== 'Enter' && keyboardEvent.key !== ' ') return; keyboardEvent.preventDefault(); const suggested = suggestedStartForDay(day); onCreate(day, undefined, suggested); }} onMouseLeave={() => setHoveredSlot(null)} onMouseMove={(mouseEvent) => { if (dragging) return; const bounds = mouseEvent.currentTarget.getBoundingClientRect(); const offset = Math.max(0, Math.min(gridHeight - 1, mouseEvent.clientY - bounds.top)); setHoveredSlot({ key, minutes: range.startMinutes + snapMinutes((offset / CALENDAR_HOUR_HEIGHT) * 60) }); }} onDragOver={(dragEvent) => { if (dragging) dragEvent.preventDefault(); }} onDrop={(dragEvent) => { dragEvent.preventDefault(); if (!dragging) return; const bounds = dragEvent.currentTarget.getBoundingClientRect(); const offset = Math.max(0, Math.min(gridHeight - 1, dragEvent.clientY - bounds.top)); const minutes = range.startMinutes + snapMinutes((offset / CALENDAR_HOUR_HEIGHT) * 60); const start = new Date(day); start.setHours(Math.floor(minutes / 60), minutes % 60, 0, 0); onMoveEvent(dragging, start); setDragging(null); setInteractionRange(null); }} className={['group relative border-r border-[#DCE5E2] focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-500', isToday ? 'bg-[#F7FCF9]' : 'bg-white'].join(' ')} style={{ height: gridHeight }}>
              {hours.map((hour) => <div key={hour} className="h-[54px] border-b border-dashed border-[#EDF2F0]" />)}
              {hoveredStart && !occupiedHover ? <button type="button" onClick={(event) => { event.stopPropagation(); onCreate(day, event.currentTarget, hoveredStart); }} className="absolute inset-x-2 z-30 flex h-9 items-center justify-center rounded-lg border border-dashed border-emerald-300 bg-emerald-50/95 text-xs font-semibold text-emerald-700 opacity-0 shadow-sm transition-all group-hover:opacity-100 hover:border-emerald-400 hover:bg-emerald-100 focus:opacity-100 focus:outline focus:outline-2 focus:outline-emerald-600 focus:outline-offset-2" style={{ top: Math.max(3, (((hovered?.minutes || range.startMinutes) - range.startMinutes) / 60) * CALENDAR_HOUR_HEIGHT + 2) }} aria-label={`Ajouter un rendez-vous le ${day.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric' })} à ${String(Math.floor((hovered?.minutes || range.startMinutes) / 60)).padStart(2, '0')} h ${String((hovered?.minutes || range.startMinutes) % 60).padStart(2, '0')}`}>＋ Ajouter un rendez-vous</button> : null}
