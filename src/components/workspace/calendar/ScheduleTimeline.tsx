    ...
    <section id="workspace-section-calendar" className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.025)] sm:p-5 lg:min-h-[70vh]">
      ...
      <div ref={scrollRef} className="mt-4 overflow-x-auto overflow-y-visible pb-1">
        <div className={['grid min-w-[760px]', view === 'jour' ? 'grid-cols-[56px_minmax(0,1fr)]' : 'grid-cols-[56px_repeat(7,minmax(120px,1fr))]'].join(' ')}>
          ...
          {days.map((day) => { const isToday = isSameDay(day, now); const dateLabel = day.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' }); return <div key={dayKey(day)} className={['sticky top-0 z-30 border-b border-r border-[#DCE5E2] px-2 pb-2 text-center', isToday ? 'bg-[#EDF9F2]' : 'bg-white'].join(' ')}><p className={['text-[10px] font-bold uppercase tracking-wide', isToday ? 'text-emerald-700' : 'text-slate-400'].join(' ')}>{day.toLocaleDateString('fr-FR', { weekday: 'short' })}</p><p className={['mt-1 flex items-center justify-center gap-1 text-sm font-semibold', isToday ? 'text-emerald-950' : 'text-slate-800'].join(' ')}>{day.getDate()}{isToday ? <span className="size-1.5 rounded-full bg-emerald-500" aria-label="Aujourd’hui" /> : null}</p><button type="button" onClick={(event) => onCreate(day, event.currentTarget, suggestedStartForDay(day))} aria-label={`Ajouter un rendez-vous le ${dateLabel}`} className={['mt-1.5 inline-flex min-h-8 items-center gap-1 rounded-md px-1.5 py-1 font-semibold text-emerald-700 transition-colors hover:bg-emerald-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-500', view === 'semaine' ? 'whitespace-nowrap text-[8px] tracking-[-0.02em]' : 'text-[10px]'].join(' ')}><Plus className={view === 'semaine' ? 'size-2.5' : 'size-3'} aria-hidden="true" />Ajouter un rendez-vous</button></div>; })}
