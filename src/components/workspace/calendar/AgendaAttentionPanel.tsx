...
export default function AgendaAttentionPanel({ events, conflicts, onOpen }: Props) {
  ...
  const pendingPresentation = getAppointmentStatusPresentation({ status: 'pending', source: null })

  return <section className="rounded-2xl border border-slate-200 bg-white p-4">
    <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">À traiter</p>
    <div className="mt-3 space-y-3 text-sm">
      {conflicts > 0 ? <p className="flex items-center gap-2 font-semibold text-amber-800"><AlertTriangle className="size-4" />{conflicts} conflit{conflicts > 1 ? 's' : ''} horaire{conflicts > 1 ? 's' : ''}</p> : null}
      {pending.length > 0 ? <button type="button" onClick={() => onOpen(pending[0])} className="flex w-full items-center justify-between text-left text-slate-700 hover:text-emerald-700"><span className={['rounded-full px-2 py-1 text-xs font-semibold', pendingPresentation.badgeClassName].join(' ')}>{pending.length} confirmation{pending.length > 1 ? 's' : ''} en attente</span><span className="font-semibold">Préparer</span></button> : null}
      ...
      {!conflicts && !pending.length && !unassigned.length ? <p className="flex items-center gap-2 text-emerald-700"><CircleCheck className="size-4" />Tout est à jour.</p> : null}
    </div>
    {first ? <button type="button" onClick={() => onOpen(first)} className="mt-3 text-xs font-semibold text-emerald-700">Ouvrir le rendez-vous</button> : null}
  </section>
}
