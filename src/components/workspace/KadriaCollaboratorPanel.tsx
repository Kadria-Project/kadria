...
const tasksQuickActions = ['Résume mes actions du jour', 'Pourquoi cet ordre ?', 'Que peux-tu préparer à ma place ?', 'Qu’est-ce qui attend ma validation ?'];

export default function KadriaCollaboratorPanel({ open, trackingActive = false, tasksActive = false, onClose }: KadriaCollaboratorPanelProps) {
  const [draft, setDraft] = useState('');
  const quickActions = tasksActive ? tasksQuickActions : trackingActive ? trackingQuickActions : defaultQuickActions;
    ...
    >
      <div className={`flex w-[420px] flex-col p-5 ${open ? '' : 'pointer-events-none'}`}>
        ...
        <section className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-emerald-950/75">{tasksActive ? 'Je peux vous aider à organiser vos actions, préparer ce qui peut l’être et garder uniquement les décisions qui nécessitent votre attention.' : trackingActive ? 'Je peux vous aider à comprendre votre pipeline et à préparer les prochaines décisions.' : 'Je peux vous aider à garder le fil de votre journée et à préparer les prochaines actions utiles.'}</p>
        </section>
