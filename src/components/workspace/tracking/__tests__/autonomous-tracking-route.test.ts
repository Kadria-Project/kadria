...
test('suivi remains independent from ArtisanDashboard', async () => { const legacyDashboard = normalize(join(root, 'src/components/ArtisanDashboard.tsx')); assert.equal(existsSync(legacyDashboard), false); assert.equal((await graph(page)).has(legacyDashboard), false) })
