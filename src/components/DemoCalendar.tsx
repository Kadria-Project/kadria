...
interface Props {
  ...
  onCreateEvent: (event: Omit<CalendarEvent, 'id'> & { status: string }) => void
  onUpdateEvent: (id: string, fields: Partial<CalendarEvent>) => void
...
export default function DemoCalendar({ events: demoEvents, onCreateEvent, onUpdateEvent, onDeleteEvent }: Props) {
        ...
        <div style={{
          ...
          <div style={{
            ...
            {selectedEvent && selectedEvent.status !== 'Fait' && (
              <button
                onClick={async () => {
                  onUpdateEvent(selectedEvent.id, { status: 'Fait' })
                  setShowModal(false)
                }}
                style={{
                  width: '100%',
